module.exports = (client, message) => {
    ...
    if (cmd && DJ.enabled && DJ.commands.includes(cmd.name)) {
        ...
        if (!message.member._roles.includes(roleDJ.id)) {
            return message.channel.send(`Esse comando é exclusivo para ${DJ.roleName} usar no server ${message.author}... quer tentar novamente ? ❌`);
        }
    }

    if (cmd && cmd.voiceChannel) {
        if (!message.member.voice.channel) return message.channel.send(`Voce não esta em um canal de voz ${message.author}... quer tentar novamente ? ❌`);
